...
export default function SplashCursor({
    ...
    useEffect(() => {
        ...
        function updateCanvasPosition() {
            if (boundaryElement && canvas && canvas.parentElement) {
                const rect = boundaryElement.getBoundingClientRect();
                canvas.parentElement.style.position = "absolute";
                canvas.parentElement.style.top = `${rect.top}px`;
                canvas.parentElement.style.left = `${rect.left}px`;
                canvas.parentElement.style.width = `${rect.width}px`;
                canvas.parentElement.style.height = `${rect.height}px`;
                canvas.parentElement.style.zIndex = "50";
            }
        }
    ...
    `,
        ...
        function resizeCanvas() {
            ...
            if (boundaryElement) {
                const rect = boundaryElement.getBoundingClientRect();
                width = scaleByPixelRatio(rect.width);
                height = scaleByPixelRatio(rect.height);
            } else {
    ...
    return (
        <div
            className={`z-50 pointer-events-none ${boundaryElement ? "absolute" : "fixed top-0 left-0 w-full h-full"}`}
        >
            <canvas
                ref={canvasRef}
                id="fluid"
